...
SCRIPT='armv7-install.sh'
NAME='WiFi Connect armv7 Installer'
...
activate_network_manager() {
    if [ "$(service_load_state NetworkManager)" = "not-found" ]; then
        ...
        ensure apt-get install -y -d network-manager

        disable_dhcpcd

        say 'Installing NetworkManager...'

        ensure apt-get install -y network-manager

        ensure apt-get clean
    else
        ...
        if [ "$(service_active_state NetworkManager)" = "active" ]; then
            ...
            ensure systemctl enable NetworkManager

            ensure systemctl start NetworkManager
        fi
    fi
...
disable_dhcpcd() {
    if [ "$(service_active_state dhcpcd)" = "active" ]; then
        ...
        ensure systemctl stop dhcpcd

        ensure systemctl disable dhcpcd
...
install_wfc() {
    ...
    ensure mv "$_download_dir/wifi-connect" $INSTALL_BIN_DIR

    ensure mkdir -p $INSTALL_UI_DIR

    ensure rm -rdf $INSTALL_UI_DIR
